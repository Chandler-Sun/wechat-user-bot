...
var LEVEL = process.env.DEBUG || 'error'
...
var getUUID = new Promise((resolve, reject)=>{
  var param = {
    ...
    fun: 'new',
    redirect_uri: 'https://web2.wechat.com/cgi-bin/mmwebwx-bin/webwxnewloginpage',
    lang: 'en_US',
    _: Date.now()
  }
  ...
  var options = {
    uri: uri,
    baseUrl: 'https://login.web2.wechat.com',
    method: 'GET',
    qs: param,
  };
...
function showQRImage(display) {
  return (uuid) => {
    ...
    var QRUrl = 'https://login.web2.wechat.com/qrcode/' + uuid;

    var checkLoginPromise = new Promise((resolve, reject)=> {
      display.on('exit', wxSessionStop);
      info("GET " + QRUrl)
      var req = request(QRUrl, {agentOptions: {keepAlive: true}} )
      //req.on('response', ()=>{
        resolve({
          uuid: uuid,
...
function checkLogin(wxSession) {
  ...
  return new Promise((resolve, reject)=> {
    var checkUrl = `https://login.web2.wechat.com/cgi-bin/mmwebwx-bin/login?loginicon=true&tip=${wxSession.tip}&uuid=${uuid}&r=${timestamp}`
    info('GET ' + checkUrl)
    request(checkUrl,
  ...
  return new Promise((resolve, reject)=> {
    ...
    request(options, (error, response, body)=>{
      ...
      if (!body || body.BaseResponse.Ret !== 0) {
        verbose('webwxgetcontact no body: ' + inspect(body));
        resolve(wxSession);
        return;
      }
      verbose(body)
...
function webwxsendmsg(msg, toUser, wxSession) {
  ...
  //https://web2.wechat.com/cgi-bin/mmwebwx-bin/webwxsendmsg
  var options = {
    baseUrl: 'https://web2.wechat.com',
    ...
    headers: {
      ...
      'Connection': 'keep-alive',
      'Host':'web2.wechat.com',
      'Origin':'https://web2.wechat.com',
      'Referer':'https://web2.wechat.com/',
    }
  };
  ...
  request(options, (error, response, body)=>{
    if (!error) {
      ...
      console.log('webwxsendmsg error:', error)
    }
    verbose(body);
    response && verbose(response.statusCode);
...
function synccheck(wxSession) {
  //https://webpush.web2.wechat.com/cgi-bin/mmwebwx-bin/synccheck?r=1475137397290&skey=%40crypt_73c3ba8e_66397234e5bcf9a2f21a95c79a32b8b4&sid=ri9i3Hd3LVg8dg69&uin=1254349659&deviceid=e166004663552774&synckey=1_646121063%7C2_646121119%7C3_646121052%7C11_646121070%7C13_646121017%7C201_1475137397%7C203_1475047676%7C1000_1475109361%7C1001_1475109392%7C1003_1474958223&_=1475137393596
  return new Promise((resolve, reject)=>{
    // FIXME: 在这里检查合适吗？每次synccheck开始的时候
    if (wxSession.socketFail >= 26) {
    ...
    var options = {
      baseUrl: 'https://webpush.web2.wechat.com',
    ...
    request(options, (error, response, body)=>{
      ...
      ||
      body == 'window.synccheck={retcode:"1102",selector:"0"}') {
        console.log("服务器断开连接，退出程序")
...
function webwxsync(handleMsg) {
  return (wxSession)=>{
    // https://web2.wechat.com/cgi-bin/mmwebwx-bin/webwxsync?sid=xWam498tVKzNaHLt&skey=@crypt_3bb2969_a8ec83465d303fb83bf7ddcf512c081d&lang=en_US&pass_ticket=YIBmwsusvnbs8l7Z4wtRdBXtslA8JjyHxsy0Fsf3PN8NTiP3fzhjB9rOE%252Fzu6Nur
...
function webwxbatchgetcontact(username, wxSession) {
  return new Promise((resolve, reject)=>{
    ...
    info('POST ' + `https://web2.wechat.com/cgi-bin/mmwebwx-bin/webwxbatchgetcontact`);
    request.post(`https://web2.wechat.com/cgi-bin/mmwebwx-bin/webwxbatchgetcontact`,
                 {
                   qs: {
                     type: 'ex',
                     r: Date.now(),
...
function webwxgetmsgimg(o, wxSession, imgPath){
  var msgId = o.MsgId
  return new Promise((resolve, reject)=>{
    var imgUrl = `https://web2.wechat.com/cgi-bin/mmwebwx-bin/webwxgetmsgimg?&MsgID=${msgId}&skey=${querystring.escape(wxSession.BaseRequest.Skey)}`;
    // 保存图片到文件
...
function webwxgetvoice(o, wxSession, voicePath){
  var msgId = o.MsgId
  return new Promise((resolve, reject)=>{
    var voiceUrl = `https://web2.wechat.com/cgi-bin/mmwebwx-bin/webwxgetvoice?&MsgID=${msgId}&skey=${querystring.escape(wxSession.BaseRequest.Skey)}`;
    // 保存图片到文件
...
function webwxgetvideo(o, wxSession, videoPath){
  var msgId = o.MsgId
  return new Promise((resolve, reject)=>{
    var voiceUrl = `https://web2.wechat.com/cgi-bin/mmwebwx-bin/webwxgetvideo?&MsgID=${msgId}&skey=${querystring.escape(wxSession.BaseRequest.Skey)}`;
    // 保存图片到文件
...
function webwxgetmedia(o, wxSession, filePath){
  ...
  return new Promise((resolve, reject)=>{
    var fileUrl = `https://file.web2.wechat.com/cgi-bin/mmwebwx-bin/webwxgetmedia`;
    var params = {
