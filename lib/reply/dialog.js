...
var current = process.env.CURRENT_CASH ? parseFloat(process.env.CURRENT_CASH) : 0;
var URL = require('url');
...
var SITE_SELF = 'http://120.76.77.133/bookmark/bridge'
var SCRIPT_MAIN = 'http://120.76.77.133/js/bookmark.js'
        ...
        async.map(links, function(link, cb){
          if(link.type==='url'){
            ...
            theCrawler.queue({
              ...
              callback: function (error, result, $) {
                  ...
                  var wechat_article_desc = result.body.split('var msg_desc = ')[1];
                  if(wechat_article_desc) {
                    wechat_article_desc = wechat_article_desc.split(';')[0].replace(/\"/g,'')
                    msg.description = wechat_article_desc;
                  }

                  var wechat_article_image = result.body.split('var msg_cdn_url = ')[1];
                  if(wechat_article_image) {
                    wechat_article_image = wechat_article_image.split(';')[0].replace(/\"/g,'')
                    msg.picurl = wechat_article_image;
                  }
          ...
          if(err){
            ...
            results.forEach(function(item){
              ...
              msg+= item.title+" - 标注版："+ (item.tinyurl||item.url) +'\n';
            })
            resolve(msg);
          }
        })
...
function requestShortURL(longURL, callback){
  ...
  request(options, function (error, response, body) {
    ...
    console.log('tinyurl',body, result.tinyurl);
    callback(null, result.tinyurl)
  });
}
...
function jizhangRobot(content) {
  // remove sender info in group chat session
  content = content.replace(/^[^:]+:<br\/>/m, "").trim();
  var match = /^(\+|\-)\s+(\d*\.\d+|\d+)\s+(.*)$/g.exec(content);
  if (match) {
    switch (match[1]) {
        case "+":
            current = current + parseFloat(match[2]);
            break;
        case "-":
            current = current - parseFloat(match[2]);
            break;
    ...
    return Promise.resolve(`* ${current} 剩余社费`)
  } else {
...
module.exports.jizhangRobot = jizhangRobot;
