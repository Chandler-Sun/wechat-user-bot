...
function sendAPNs(job, url, wxSession){
  ...
  if(uniqUserTokens.length){
    ...
    apnMsg.alert = msg
    apnMsg.payload = {'url': url, 'resource_key':job.data.msg.resource.key}
...
function messageQueue(wxSession) {
  ...
  return new Promise((resolve, reject)=> {
    ...
    messageQueueWeChat.process(function(job, done) {
      if(job.data.source){
        var longURL = util.format('%s?key=%s&group_id=%s&source=%s&annotation_id=%s',
          SITE_SELF,
          encodeURIComponent(job.data.msg.resource.key),
